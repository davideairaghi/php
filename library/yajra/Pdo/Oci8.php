...
class Oci8 extends PDO {
	...
	public function __construct($dsn, $username, $password, array $options = array())
	{
		...
		$st = oci_parse($this->_dbh,'ALTER SESSION SET NLS_COMP=LINGUISTIC');
		oci_execute($st);
		oci_free_statement($st);
		
		$st = oci_parse($this->_dbh,'ALTER SESSION SET NLS_SORT=BINARY_CI');
		oci_execute($st);
		oci_free_statement($st);
	...
	public function prepare($statement, $options = null)
	{
		...
		return new Statement($sth, $this, $statement, $options);
	}
