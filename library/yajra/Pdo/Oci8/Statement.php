...
class Statement extends PDOStatement {
	 ...
	 */
	public function __construct($sth, Oci8 $pdoOci8, $query, array $options = array())
	{
	...
	public function fetch($fetchMode = null,$cursorOrientation = PDO::FETCH_ORI_NEXT,$cursorOffset = 0)
	{
		...
		switch ($fetchMode)
		{
			...
			default:
				...
				$x = 'SELECT COUNT(*) "numrows"';
				$l = strlen($x);
				if (substr($this->_query,0,$l)==$x) {
	...
	public function bindParam($parameter,&$variable,$dataType = PDO::PARAM_STR,$maxLength = -1,$options = null)
	{
		...
		// se abbiamo "autoparams" arriviamo da insert o simili e dobbiamo "scalare" gli indici di posizione ....
		if (strpos($this->_query,':autoparam')!==false) {
			if (!preg_match('#:autoparam'.$parameter.'[),.]#',$this->_query)) {
				-- $parameter;
			}
		}
		
		// se trovo dei parametri chiamati APLxxx li salto ...
		if (preg_match('#^APL([0-9]+)$#',$parameter)) {
			return ;
		}
		...
		// echo $parameter.' '.print_r($this->_bindParams,true).'<br>';

		// se non ho in lista il parametro esco ...
		if (preg_match('#^:autoparam([0-9]+)$#',$parameter) && !in_array($parameter,$this->_bindParams,true)) {
			$parameter = str_replace('autoparam','',$parameter);
		}
